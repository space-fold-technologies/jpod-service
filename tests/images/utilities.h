...
{
    using destination = std::shared_ptr<download_destination>;
    ...
    inline auto download_invoked = [](const std::string &path, const std::map<std::string, std::string> &headers, destination sink, report_callback callback)
    {
