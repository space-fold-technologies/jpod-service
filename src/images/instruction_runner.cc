#include <images/instruction_runner.h>
...
namespace images
{
    ...
    void InstructionRunner::initialize()
    {
        ...
        else if (pid == 0)
        {
            ...
            if (auto err = execlp(target_shell, target_shell, "-c", fmt::format("\"{}\"", instruction), NULL); err < 0)
            {
                listener.on_instruction_runner_completion(this->id, std::error_code(errno, std::system_category()));
            }
            else
        ...
        if (int flags = fcntl(fd, F_GETFL); flags != -1)
        {
            ...
            listener.on_instruction_runner_initialized();
        }
        listener.on_instruction_runner_completion(this->id, std::error_code(errno, std::system_category()));
    }
    void InstructionRunner::process_wait()
    {
        pid_t pid;
        int stat;
        do
    ...
    bool InstructionRunner::setup_pipe(int fd)
    {
    ...
    void InstructionRunner::read_from_shell()
    {
        in->async_read_some(
            asio::buffer(buffer),
            [this](const std::error_code &err, std::size_t bytes_transferred)
            {
                if (!err)
                {
                    this->listener.on_instruction_runner_data_received(buffer);
                    this->in->async_wait(
                        asio::posix::stream_descriptor::wait_read,
                        [this](const std::error_code &err)
                        {
    ...
    bool InstructionRunner::close_on_exec(int fd)
    {
    ...
    void InstructionRunner::disable_stdio_inheritance()
    {
