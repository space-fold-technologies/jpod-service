...
namespace domain::networking
{
    ...
    private:
        ...
        bool add_interface(const int socket, const std::string &type, const std::string &name, std::error_code &error);
        bool rename_interface(const int socket, ifreq &request, const std::string &name, std::error_code &error);
        bool set_addresses(int socket, const std::string &name, const std::vector<ip_address> &addresses, std::error_code &error);
